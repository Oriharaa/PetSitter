...
<%
	ProBoardVO proboard = (ProBoardVO)request.getAttribute("vo");
...
//세션 종료시 홈으로
  if(session.getAttribute("id") == null) {
	...
	if(proboard.getSECRET_CHECK().equals("Y") && !(proboard.getMEMBER_ID().equals(id))) {
		out.println("<script>");
...
<style>
  ...
  <body data-spy="scroll" data-target=".site-navbar-target" data-offset="300">
			...
			<div class="row justify-content-center">
				<div class="col-md-10">
					<table class="table table-sm">
						<tr>
							<td>제목</td>
							<td><%=proboard.getPRO_SUBJECT()%></td>
							</tr> 
						<tr>
							<td>작성일</td>
							<td><%=format1.format(proboard.getPRO_DATE()) %></td>
						</tr>
						<tr> 
							<td>작성자</td>
							<td><%=proboard.getMEMBER_NICKNAME() %></td>
						</tr>
						<tr>
							<td>조회수</td>
							<td><%=proboard.getPRO_READCOUNT() %></td>
						</tr>
						<tr>
							<td>첨부파일</td>
							<td>
									<%if(!(proboard.getPRO_ORG_FILE() == null)) { 
										
										//파일 추출
										String pof = proboard.getPRO_ORG_FILE();	
										String[] proOrgFile = pof.split(",");
												
										for(int i=0;i<proOrgFile.length;i++) {
										System.out.println(proOrgFile[i]);
										}
										
										String puf = proboard.getPRO_UP_FILE();	
										String[] proUpFile = pof.split(",");
										
										for(int i=0;i<proUpFile.length;i++) {
											System.out.println(proUpFile[i]);
											}
									%>
									<a href="./filedownload.bo?num=<%=proOrgFile[0]%>&of=<%=proUpFile[0]%>&of2=<%=proOrgFile[0]%>">
										<%=proboard.getPRO_ORG_FILE() %>
									</a>
									
									<%if(proOrgFile[1] != null){ %>
					...
					<div class="col-md-10">
						<table class="table table-borderless">
							<tr>
								<td><%=proboard.getPRO_CONTENT() %></td>
							</tr>
							<tr>
								<td>
									<img src="./upload/<%=proboard.getPRO_UP_FILE() %>">
								</td> 
							</tr>
					</table>
					
			<%if((proboard.getMEMBER_ID().equals(id)) || rank.equals("admin") || rank.equals("manager")) {%>
				<a type="button" style="background:#53dc98;" class="btn btn-sm" id="btnModify" href="./mboardmodifyform.me?num=<%=proboard.getPRO_NUM() %>">수정</a>
	  		<a type="button" style="background:#53dc98;" class="btn btn-sm" id="btnDelete" href="./mboardDelete.me?num=<%=proboard.getPRO_NUM() %>">삭제</a>
  	 		...
  	 		<a type="button" class="btn btn-sm btn-danger float-right" id="btnReportOpen">신고</a>
  	 		 ...
  	 		 <form action="./reportArticle.me" method="post" name="reportform">
  	 		  	  <input type="hidden" name="MEMBER_ID" value="${id}">
						  <input type="hidden" name="MEMBER_NUM" value="<%=proboard.getPRO_NUM()%>">

  	 		<!-- 신고 창 시작 -->
	 		 		<div id="modal">
	 		 			<div class="modal_report">
		 					<h3>신고하기</h3>
	 		 				신고 글 번호 : <%=proboard.getPRO_NUM() %><br/>
	 		 				<p>신고자 : ${name} ${id}</p>
	 		 				<p>신고 사유</p>
	...
	<jsp:include page="./proboard_comments.jsp">
	<jsp:param name="bno" value="<%=proboard.getPRO_NUM() %>"/>
</jsp:include>
